...
### 模块 13: 高级扩展性与可观测性 (Module 13: Advanced Scalability) [DONE]

- **路由模块化:** 拆分 HTTP/gRPC 路由，解耦 `main.go`.
- **异步任务队列 (Asynq):** 引入 Redis 队列处理邮件发送、交易广播等后台任务.
- **业务监控:** Prometheus 埋点 (注册人数, 充值金额, 提现处理时间).

### 模块 14: 微服务拆分实战 (Module 14: Microservices) [DONE]

**目标:** 将单体架构拆分为松耦合的微服务集群.

- **User Service:** gRPC, 用户与账户管理.
- **Wallet Service:** gRPC, 地址生成与资金逻辑.
- **API Gateway:** HTTP (Gin), 统一入口与鉴权.
- **Broadcaster:** 独立进程 (Worker), 物理隔离私钥签名环境 (最为关键的安全升级).
- **事件驱动 (Kafka):** 引入 Kafka 替代 Redis Streams，解耦服务间通信.

### 模块 15: 压力测试 (Module 15: Load Testing) [DONE]

- **工具:** k6 (JavaScript 脚本).
- **场景:** 高并发注册, 充值回调轰炸.
- **基准:** 确立系统吞吐量瓶颈 (TPS).

### 模块 16: 安全审计 (Module 16: Security Audit) [DONE]

- **漏洞扫描:** `govulncheck` 扫描依赖树.
- **代码审计:** 检查 SQL 注入, 越权访问, 私钥硬编码.

### 模块 17: MPC 钱包服务化 (Module 17: MPC Service) [PLANNED]

_这是从单纯的 "私钥管理" 进阶到 "分布式托管" 的关键一步。_

**Web3 重点:** 阈值签名方案 (TSS), 分布式密钥生成 (DKG).
**Web2 集成点:**

- **gRPC 服务:** 将 CLI 逻辑封装为高性能 RPC 服务.
- **分布式协调:** 多个 MPC 节点如何同步状态 (Coordinator).

### 模块 18: 多链扩展 - TRON (Module 18: Multi-Chain - TRON) [PLANNED]

**Web3 重点:** 波场协议, TRC-20 USDT, Bandwidth/Energy 模型.
**实战:** 实现 TRON 地址生成, 离线签名, USDT 归集.

### 模块 19: 基础设施自动化 (Module 19: IaC) [PLANNED]

- **Helm:** 管理 K8s 复杂部署清单.
- **Terraform:** 代码化管理云资源 (AWS/GCP).

### 模块 20: 全链路监控 (Module 20: Observability) [PLANNED]

- **ELK Stack:** 集中式日志分析.
- **Jaeger/Tempo:** 分布式链路追踪 (Tracing).

## 下一步计划

1.  **开始 Module 17:** 设计 `mpc-service` 的 Proto 接口与目录结构。
2.  **研究 TSS:** 复习 `tss-lib` 或 `cmp` 协议的 Go 实现。
